import { randomId } from "better-auth";
import { nanoid } from "nanoid";
import { query, get, run, type Database } from "./db";
...
export async function verifyNoteOwnership(
	...
	const note = await get<{ user_id: string }>(
...
export async function getNoteContent(userId: string, noteId: string): Promise<Note | null> {
	return get<Note>(
		"SELECT * FROM notes WHERE id = ? AND user_id = ? LIMIT 1",
		[noteId, userId],
	);
...
export async function setNotePublic(
	...
	if (isPublic) {
		...
		if (!slug) {
			...
			run(
				"UPDATE notes SET is_public = 1, updated_at = datetime('now') WHERE id = ?",
				[noteId],
			);
		}
	} else {
