...
Conference.fakeAttendeeWebApi = function () {
  ...
  return {
    // 서버에 attendee를 POST 전송하는 척한다. attendee 사본(마치 서버에서 새 버전을 조회해오는 것처럼)으로 귀결되는 프라미스를 반환하고, 귀결 시점에 이 레코드에는 데이터베이스에서 할당된 PK(attendeeId)가 들어있을 것이다.
    ...
    // 전체 참가자에 대한 프라미스를 반환한다. 이 프라미스는 반드시 귀결되지만, 필요한 테스트 코드에서 스파이를 심을 수도 있다.
    getAll: function getAll() {
      return new Promise(function (resolve, reject) {
        // setTimeout은 실제 조건을 흉해 내기 위해 post보다 지연 시간이 짧다.
