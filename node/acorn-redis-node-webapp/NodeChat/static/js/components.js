var RoomForm = React.createClass({
  ...
  render: function() {
    return React.DOM.div(
      ...
      React.DOM.input({type: 'text', placeholder: 'Room Name', clasName: 'form-control', ref: 'roomName'}, null),
      React.DOM.button({className: 'btn btn-primary btn-block topmargin', onClick: this.joinRoomHandler}, 'Join Room'),
      React.DOM.ul(null, this.props.rooms.map(function(r) {
        return React.DOM.li(
          {className: 'list-unstyled'}, 
          React.DOM.a({href: '#room/' + r.get('name')}, r.get('name'))
        );
      }))
    );
  }
...
var ChatMessage = React.createClass({
  render: function() {
    ...
    return React.DOM.li(
      ...
      React.DOM.div(
        {className: pull},
        UserView({user: this.props.chat.get('user'), size: 50, useName: true}),
  ...
  render: function() {
    return React.DOM.div(
      ...
      React.DOM.div(
        ...
        React.DOM.div(
          {className: 'col-sm-8 chat-list', ref: 'chatList'},
