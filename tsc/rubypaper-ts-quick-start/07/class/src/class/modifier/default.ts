// 07-1-4 접근 제한자의 사용법 - 기본 접근 제한자 - 기본 접근 제한자인 public
...
class Account {
  ...
  // 생성자 매개변수 interestRate는 public으로 설정됐으므로 호출가능
  getInterestRate() {
...
class MyAccount extends Account {
  ...
  constructor() {
    ...
    console.log(`2번) 적금: ${this.balance}원, ${this.getBalance}원 / 이율: ${this.interestRate}, ${this.getInterestRate()}$ / 은행명: ${this.bankName}`); // 2번) 적금: 2000원, 2000원 / 이율: 0.1, 0.1$ / 은행명: happy bank
  }
}

let account = new Account();
console.log(`1번) 적금: ${account.balance}원, ${account.getBalance}원 / 이율: ${account.interestRate}, ${account.getInterestRate()}`); // 1번) 적금: 0원, 0원 / 이율: 0.1, 0.1
